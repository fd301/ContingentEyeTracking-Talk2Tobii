...
%                 end
    ...
    if( DISPSACFIX )
        res = [1024 768];
        store = orgFixSac(inputPathEvents, inputPathEyeTracking, FieldNames, res );
    end
...
%        TimeLineR(1:end-1) = TimeLineR(1:end-1) - ifi;
    ...
    glMaterialfv(GL.FRONT_AND_BACK,GL.DIFFUSE, [ 0.7 0.7 0.7 0.5 ]);
    ...
    if(TEST)
        ...
        CenSY = aa*CenSX+bb;
        CenS = [CenSX CenSY];
        EpntSeg1 = CalcEllipse( CenS, a, b, f1, f2, steps, 1);
        ...
        CenSY = aa*CenSX+bb;
        CenS = [CenSX CenSY];
        EpntSeg2 = CalcEllipse( CenS, a, b, f1, f2, steps, 1);
        
        EpntSeg1L = [res(1)-EpntSeg1(:,1) EpntSeg1(:,2)];
        EpntSeg2L = [res(1)-EpntSeg2(:,1) EpntSeg2(:,2)];
        ...
        ObjPntsL = [res(1)-ObjPntsR(:,1) ObjPntsR(:,2)];
    ...
    while ( lenPlay-1 > count)
        ...
        if( AtLeastOnce || indR2>=0 || indR3>=0 || indR4>=0 || indR5>=0 || flagStart || StillUpdate || PLOT_ELLIPSE_PNTS)
            ...
            vpos = drawShape_test( objOrd, numObj, ColArray(colInd,:), ang, theta, theta2, rotatev, dotson, normalson, IndStore );
            ...
            if( PLOT_ELLIPSE_PNTS )
                ...
                if(TEST)
                    Screen('glPoint', win, [0 0 255], 490, 492, 10);
                    Screen('glPoint', win, [0 0 255], 349, 489, 10);
                    Screen('glPoint', win, [0 0 255], 323, 404, 10);

                    Screen('glPoint', win, [0 0 255], res(1)-490, 492, 10);
                    Screen('glPoint', win, [0 0 255], res(1)-349, 489, 10);
                    Screen('glPoint', win, [0 0 255], res(1)-323, 404, 10);
                    
                    for jj=1:length(EpntSeg1)
                        Screen('glPoint', win, [0 255 0], EpntSeg1(jj,1), EpntSeg1(jj,2), 2);
                    end

                    for jj=1:length(EpntSeg2)
                        Screen('glPoint', win, [255 0 0], EpntSeg2(jj,1), EpntSeg2(jj,2), 2);
                    end

                    for jj=1:length(ObjPntsR)
                        Screen('glPoint', win, [0 255 0], ObjPntsR(jj,1), ObjPntsR(jj,2), 5);
                    end
                    
                    for jj=1:length(ObjPntsL)
                        Screen('glPoint', win, [0 255 0], ObjPntsL(jj,1), ObjPntsL(jj,2), 5);
                    end
                    
                    for jj=1:length(EpntSeg1L)
                        Screen('glPoint', win, [0 255 0], EpntSeg1L(jj,1), EpntSeg1L(jj,2), 2);
                    end

                    for jj=1:length(EpntSeg2L)
                        Screen('glPoint', win, [255 0 0], EpntSeg2L(jj,1), EpntSeg2L(jj,2), 2);
                    end
        ...
        if(PLAYBACK)
            ...
            if( CAPTURESCREEN )
                ...
                for j=tmpIndex(1):tmpIndex(2)
                    ...
                    %draw saccades and fixations if it is required
                    if(DISPSACFIX&&DISPSACFIX2)                       
                        DrawSacFix(store, win,res, 1, j+1);
                    end
        ...
        if(TEST && ~DISPSACFIX)
            %found mouse coordinates
            [tmpclicks,tmpx,tmpy] = GetClicks(win)
        end
